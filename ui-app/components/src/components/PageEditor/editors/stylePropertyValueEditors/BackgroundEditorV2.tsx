...
import { RelatedProps } from './simpleEditors';
...
const BACKGROUND_STANDARD_PROPS = [
	'backgroundColor',
	'backgroundClip',
	'backgroundImage',
	'backgroundSize',
...
const DEFAULT_CLIP = 'border-box';
const DEFAULT_POSITION_X = '0px';
const DEFAULT_POSITION_Y = '0px';
const DEFAULT_SIZE_VALUE = 'auto';
const DEFAULT_LENGTH_VALUE = 'auto';

const BACKGROUND_IMAGE_TYPE_OPTIONS = [
	{ name: 'URL', displayName: 'Image' },
	{ name: 'LINEAR_GRADIENT', displayName: 'Linear' },
	{ name: 'RADIAL_GRADIENT', displayName: 'Radial' },
	{ name: 'CONIC_GRADIENT', displayName: 'Conic' },
	{ name: 'CUSTOM', displayName: 'Custom CSS' },
];

const BACKGROUND_SIZE_MODE_OPTIONS = [
...
const BACKGROUND_SIZE_ICON_OPTIONS = [
	{
		name: 'cover',
		description: 'Cover',
		icon: (
	...
	},
	{
		...
		icon: (
			<g transform="translate(9 9)">
				<rect y="5" width="9" height="9" rx="0.5" fill="#02B694" strokeOpacity={0} />
				<path
...
const BACKGROUND_REPEAT_ICON_OPTIONS = [
	{
		name: 'no-repeat',
		description: 'No repeat',
		icon: (
			<g transform="translate(9 9)">
				<rect width="14" height="14" rx="1" opacity={0} strokeOpacity={1} />
				<path
...
const BACKGROUND_CLIP_ICON_OPTIONS = [
	{
		name: 'border-box',
		description: 'Border Box',
		icon: (
			<g transform="translate(9 9)">
				<rect width="14" height="14" rx="2" strokeOpacity={0} className="_lowlight" />
				<path
...
const BACKGROUND_POSITION_PRESETS: BackgroundPositionPreset[] = [
	{
		name: 'left top',
...
const BACKGROUND_POSITION_ICON_OPTIONS = BACKGROUND_POSITION_PRESETS.map(preset => ({
	name: preset.name,
	description: preset.description,
	icon: preset.icon,
}));

const BACKGROUND_LAYER_ACTION_OPTIONS = [
	{
		name: 'Add',
...
export function BackgroundEditorV2(props: Readonly<StyleEditorsProps>) {
	if (props.isDetailStyleEditor) {
		return <BackgroundDetailedEditor {...props} />;
	}
	return <BackgroundLayeredEditor {...props} />;
}

export const BackgroundEditor = BackgroundEditorV2;

function BackgroundLayeredEditor(props: Readonly<StyleEditorsProps>) {
	const {
		subComponentName,
		iterateProps,
		pseudoState,
	...
	const { values: backgroundValues, hasExpressions } = useMemo(() => {
		const valueMap: Record<string, string> = {};
		let advanced = false;
		for (const prop of BACKGROUND_STANDARD_PROPS) {
			...
			if (extracted.value?.location && extracted.value.location.type !== 'VALUE') {
				advanced = true;
			}
			valueMap[prop] = extracted.value?.value ?? '';
		}
		return { values: valueMap, hasExpressions: advanced };
	}, [subComponentName, iterateProps, pseudoState, selectorPref, selectedComponent]);

	const [layers, setLayers] = useState<BackgroundLayer[]>(() =>
		buildLayersFromValues(backgroundValues),
	);
	const [backgroundColor, setBackgroundColor] = useState<string>(
		backgroundValues.backgroundColor ?? '',
	);
	const [fileBrowserLayerIndex, setFileBrowserLayerIndex] = useState<number | null>(null);

	useEffect(() => {
		setLayers(buildLayersFromValues(backgroundValues));
		setBackgroundColor(backgroundValues.backgroundColor ?? '');
	}, [
		backgroundValues.backgroundImage,
		backgroundValues.backgroundSize,
		backgroundValues.backgroundRepeat,
		backgroundValues.backgroundClip,
		backgroundValues.backgroundWidth,
		backgroundValues.backgroundHeight,
		backgroundValues.backgroundPosition,
		backgroundValues.backgroundPositionX,
		backgroundValues.backgroundPositionY,
		backgroundValues.backgroundColor,
	]);
	...
	const persistLayers = (nextLayers: BackgroundLayer[]) => {
		...
		valuesChangedOnlyValues({
			...
			locationHistory,
			pageExtractor,
			propValues: buildLayerPropValues(nextLayers, backgroundColor),
		});
	};

	const commitColorChange = (value: string) => {
		...
		valuesChangedOnlyValues({
			...
			locationHistory,
			pageExtractor,
			propValues: [{ prop: 'backgroundColor', value }],
		});
	};

	const updateLayerAt = (index: number, updater: (layer: BackgroundLayer) => BackgroundLayer) => {
		const nextLayers = layers.map((layer, idx) => (idx === index ? updater(layer) : layer));
		persistLayers(nextLayers);
	};

	const insertLayer = (position: number) => {
		const nextLayers = duplicate(layers);
		nextLayers.splice(position, 0, createEmptyLayer());
		persistLayers(nextLayers);
	};
	...
	const moveLayer = (from: number, to: number) => {
		...
		const nextLayers = duplicate(layers);
		const [item] = nextLayers.splice(from, 1);
		nextLayers.splice(to, 0, item);
		persistLayers(nextLayers);
	};

	const removeLayer = (index: number) => {
		if (layers.length === 1) return;
		const nextLayers = layers.filter((_, idx) => idx !== index);
		persistLayers(nextLayers);
	};

	const handleFileSelection = (url: string, directory?: boolean) => {
		if (directory || fileBrowserLayerIndex === null) return;
		updateLayerAt(fileBrowserLayerIndex, layer => ({
			...layer,
			image: { type: 'URL', value: url },
		}));
		setFileBrowserLayerIndex(null);
	};

	const renderImageInput = (layer: BackgroundLayer, index: number) => {
		if (layer.image.type === 'URL') {
			return (
				<div className="_simpleEditor">
					<div className="_inputWithIcon">
						<svg
							...
							viewBox="0 0 14 14"
							fill="none"
							onClick={() => setFileBrowserLayerIndex(index)}
						...
						<input
							...
							value={layer.image.value}
							onChange={e =>
								updateLayerAt(index, current => ({
									...current,
									image: { ...current.image, value: e.target.value },
								}))
							}
			...
			return (
				<div className="_simpleEditor">
					<textarea
						...
						placeholder="Custom CSS e.g., image-set(url(...) 1x)"
					/>
				</div>
			);
		}
		return (
			<div className="_simpleEditor">
				<textarea
					...
					placeholder="Gradient args, e.g., 90deg, #fff 0%, #000 100%"
				/>
			</div>
		);
	};

	const renderLayerCard = (layer: BackgroundLayer, index: number) => {
		const positionPreset =
			BACKGROUND_POSITION_PRESETS.find(
				preset =>
					preset.positionX === layer.positionX && preset.positionY === layer.positionY,
			)?.name ?? '';

		return (
			<div key={layer.id} className="_eachBackgroundEditor">
				<div className="_combineEditors">
					<div className="_layerControls _column" style={{ display: 'flex' }}>
						<div className="_combineEditors">
							<div className="_simpleLabel">Layer {index + 1}</div>
							<IconsSimpleEditor
								options={BACKGROUND_LAYER_ACTION_OPTIONS}
								onChange={action => {
									if (action === 'Add') insertLayer(index + 1);
									else if (action === 'Move Up') moveLayer(index, index - 1);
									else if (action === 'Move Down') moveLayer(index, index + 1);
									else if (action === 'Delete') removeLayer(index);
								}}
						...
						<ButtonBar
							value={layer.image.type}
							onChange={v =>
								updateLayerAt(index, current => ({
									...current,
									image: { type: v as BackgroundImageType, value: '' },
								}))
							}
							options={BACKGROUND_IMAGE_TYPE_OPTIONS}
						/>
					</div>
				</div>
				{renderImageInput(layer, index)}
				<div className="_simpleEditorGroup">
					<div className="_simpleEditorGroupContent">
						<div className="_editorLine">
							<span className="_label">Repeat</span>
							<IconsSimpleEditor
								options={BACKGROUND_REPEAT_ICON_OPTIONS}
								selected={layer.repeat}
								onChange={value =>
									updateLayerAt(index, current => ({
										...current,
						...
						<div className="_editorLine">
							<span className="_label">Clip</span>
							<IconsSimpleEditor
								options={BACKGROUND_CLIP_ICON_OPTIONS}
								selected={layer.clip}
								onChange={value =>
									updateLayerAt(index, current => ({
										...current,
										clip: value as string,
									}))
								}
								withBackground={true}
							/>
						...
						<div className="_editorLine">
							<span className="_label">Size</span>
							<ButtonBar
								value={layer.sizeMode}
								onChange={value =>
									updateLayerAt(index, current => ({
										...current,
										sizeMode: value as BackgroundSizeMode,
									}))
								}
								options={BACKGROUND_SIZE_MODE_OPTIONS}
							/>
						</div>
						{layer.sizeMode === 'custom' ? (
							<React.Fragment>
								<div className="_editorLine">
									<span className="_label">Width</span>
									<PixelSize
										value={layer.width}
										onChange={value =>
											updateLayerAt(index, current => ({
												...current,
												width: value || DEFAULT_LENGTH_VALUE,
											}))
										}
										placeholder="auto"
									/>
								...
								<div className="_editorLine">
									<span className="_label">Height</span>
									<PixelSize
										value={layer.height}
										onChange={value =>
											updateLayerAt(index, current => ({
												...current,
												height: value || DEFAULT_LENGTH_VALUE,
											}))
										}
										placeholder="auto"
									/>
						...
						<div className="_editorLine">
							<span className="_label">Position</span>
							<IconsSimpleEditor
								options={BACKGROUND_POSITION_ICON_OPTIONS}
								selected={positionPreset}
								onChange={value => {
									const preset = BACKGROUND_POSITION_PRESETS.find(
										p => p.name === value,
									);
									if (!preset) return;
									updateLayerAt(index, current => ({
						...
						<div className="_editorLine">
							<span className="_label">Horizontal</span>
							<PixelSize
								value={layer.positionX}
								onChange={value =>
									updateLayerAt(index, current => ({
										...current,
										positionX: value || DEFAULT_POSITION_X,
									}))
								}
								placeholder="0%"
							/>
						...
						<div className="_editorLine">
							<span className="_label">Vertical</span>
							<PixelSize
								value={layer.positionY}
								onChange={value =>
									updateLayerAt(index, current => ({
										...current,
										positionY: value || DEFAULT_POSITION_Y,
									}))
								}
								placeholder="0%"
							/>
	...
	const fileBrowserPopup =
		fileBrowserLayerIndex !== null ? (
			<div className="_popupBackground" onClick={() => setFileBrowserLayerIndex(null)}>
				<div
					className="_popupContainer _imagePopupContainer"
					onClick={e => e.stopPropagation()}
				>
					<FileBrowser
						selectedFile={layers[fileBrowserLayerIndex]?.image.value ?? ''}
						onChange={(url: string, type: string, directory: boolean) => {
							if (directory) return;
	...
	return (
		<>
			<div className="_simpleEditor">
				<div className="_simpleLabel">Background Color</div>
				<CommonColorPickerPropertyEditor
					color={{
						value: backgroundColor,
						location: { type: 'VALUE' },
					}}
					onChange={(color: ComponentProperty<string>) =>
						commitColorChange(color.value ?? '')
					}
				/>
			</div>
			{layers.map((layer, index) => renderLayerCard(layer, index))}
			{fileBrowserPopup}
		</>
	);
}

function createEmptyLayer(): BackgroundLayer {
	return {
		id: shortUUID(),
		...
		sizeMode: 'auto',
		width: DEFAULT_LENGTH_VALUE,
		height: DEFAULT_LENGTH_VALUE,
	};
}

function buildLayersFromValues(values: Record<string, string>): BackgroundLayer[] {
	const imageValues = parseBackgroundImages(values.backgroundImage);
	const repeatValues = splitCSSValueList(values.backgroundRepeat);
	const clipValues = splitCSSValueList(values.backgroundClip);
	const sizeValues = splitCSSValueList(values.backgroundSize);
	const widthValues = splitCSSValueList(values.backgroundWidth);
	const heightValues = splitCSSValueList(values.backgroundHeight);
	const positionValues = splitCSSValueList(values.backgroundPosition);
	const positionXValues = splitCSSValueList(values.backgroundPositionX);
	const positionYValues = splitCSSValueList(values.backgroundPositionY);
	...
	const layerCount = Math.max(
		...
		positionXValues.length || 0,
		positionYValues.length || 0,
	);
	...
	for (let i = 0; i < layerCount; i++) {
		...
		if (imageValues[i]) layer.image = imageValues[i];
		if (repeatValues[i]) layer.repeat = repeatValues[i];
		if (clipValues[i]) layer.clip = clipValues[i];

		const sizeInfo = parseBackgroundSizeValue(sizeValues[i], widthValues[i], heightValues[i]);
		layer.sizeMode = sizeInfo.mode;
		layer.width = sizeInfo.width;
		layer.height = sizeInfo.height;

		const parsedPosition = parseBackgroundPositionValue(positionValues[i]);
		layer.positionX = positionXValues[i] ?? parsedPosition.x ?? DEFAULT_POSITION_X;
		layer.positionY = positionYValues[i] ?? parsedPosition.y ?? DEFAULT_POSITION_Y;

		layers.push(layer);
	}

	return layers.length ? layers : [createEmptyLayer()];
}

function buildLayerPropValues(layers: BackgroundLayer[], backgroundColor: string) {
	const images = layers.map(layer => formatBackgroundImage(layer.image));
	const repeats = layers.map(layer => layer.repeat || DEFAULT_REPEAT);
	const clips = layers.map(layer => layer.clip || DEFAULT_CLIP);
	const positions = layers.map(layer => formatPosition(layer.positionX, layer.positionY));
	const positionX = layers.map(layer => layer.positionX || DEFAULT_POSITION_X);
	const positionY = layers.map(layer => layer.positionY || DEFAULT_POSITION_Y);
	const sizes = layers.map(layer => formatBackgroundSize(layer));
	const widths = layers.map(layer =>
		layer.sizeMode === 'custom' ? layer.width || DEFAULT_LENGTH_VALUE : 'auto',
	);
	const heights = layers.map(layer =>
		layer.sizeMode === 'custom' ? layer.height || DEFAULT_LENGTH_VALUE : 'auto',
	);

	const joinValues = (list: string[], fallback: string) =>
		list.map(value => (value?.trim().length ? value.trim() : fallback)).join(', ');

	return [
		{ prop: 'backgroundImage', value: joinValues(images, 'none') },
		{ prop: 'backgroundRepeat', value: joinValues(repeats, DEFAULT_REPEAT) },
		{ prop: 'backgroundClip', value: joinValues(clips, DEFAULT_CLIP) },
		{
			prop: 'backgroundPosition',
			value: joinValues(positions, formatPosition(DEFAULT_POSITION_X, DEFAULT_POSITION_Y)),
		},
		{ prop: 'backgroundPositionX', value: joinValues(positionX, DEFAULT_POSITION_X) },
		{ prop: 'backgroundPositionY', value: joinValues(positionY, DEFAULT_POSITION_Y) },
		{ prop: 'backgroundSize', value: joinValues(sizes, DEFAULT_SIZE_VALUE) },
		{ prop: 'backgroundWidth', value: joinValues(widths, DEFAULT_LENGTH_VALUE) },
		{ prop: 'backgroundHeight', value: joinValues(heights, DEFAULT_LENGTH_VALUE) },
		{ prop: 'backgroundColor', value: backgroundColor },
	];
}
...
function parseBackgroundImages(value?: string): BackgroundImageValue[] {
	return splitCSSValueList(value).map(entry => {
		...
		if (trimmed.toLowerCase().startsWith('url(')) {
			...
			return { type: 'URL', value: trimQuotes(content) };
		}
		if (trimmed.toLowerCase().startsWith('linear-gradient(')) {
			return { type: 'LINEAR_GRADIENT', value: stripFunctionArgs(trimmed) };
		}
		if (trimmed.toLowerCase().startsWith('radial-gradient(')) {
			return { type: 'RADIAL_GRADIENT', value: stripFunctionArgs(trimmed) };
		}
		if (trimmed.toLowerCase().startsWith('conic-gradient(')) {
			return { type: 'CONIC_GRADIENT', value: stripFunctionArgs(trimmed) };
		}
		return { type: 'CUSTOM', value: trimmed };
	});
}

function parseBackgroundSizeValue(
	sizeValue?: string,
	widthValue?: string,
	heightValue?: string,
): { mode: BackgroundSizeMode; width: string; height: string } {
	const trimmed = sizeValue?.trim();
	if (trimmed === 'cover' || trimmed === 'contain') {
		return {
			mode: trimmed,
			width: widthValue || DEFAULT_LENGTH_VALUE,
			height: heightValue || DEFAULT_LENGTH_VALUE,
		};
	}
	...
	return {
		mode: 'custom',
		width: widthValue || parts[0] || DEFAULT_LENGTH_VALUE,
		height: heightValue || parts[1] || DEFAULT_LENGTH_VALUE,
	};
}

function parseBackgroundPositionValue(value?: string): { x: string; y: string } {
	if (!value) return { x: DEFAULT_POSITION_X, y: DEFAULT_POSITION_Y };
	const parts = value.split(/\s+/).filter(Boolean);
	return {
		x: parts[0] || DEFAULT_POSITION_X,
		y: parts[1] || DEFAULT_POSITION_Y,
	};
}

function formatBackgroundImage(image: BackgroundImageValue): string {
	switch (image.type) {
		case 'URL': {
		...
		case 'LINEAR_GRADIENT':
			return `linear-gradient(${image.value})`;
		case 'RADIAL_GRADIENT':
			return `radial-gradient(${image.value})`;
		case 'CONIC_GRADIENT':
			return `conic-gradient(${image.value})`;
		case 'CUSTOM':
		default:
...
function formatBackgroundSize(layer: BackgroundLayer): string {
	if (layer.sizeMode === 'cover' || layer.sizeMode === 'contain') {
		return layer.sizeMode;
	}
	if (layer.sizeMode === 'custom') {
		const width = layer.width || DEFAULT_LENGTH_VALUE;
		const height = layer.height || DEFAULT_LENGTH_VALUE;
		return `${width} ${height}`.trim();
	}
	return DEFAULT_SIZE_VALUE;
}

function formatPosition(x: string, y: string): string {
	return `${x || DEFAULT_POSITION_X} ${y || DEFAULT_POSITION_Y}`.trim();
}
