...
/**
 * Get the base URL prefix for static assets (styleProperties, css)
 * - If cdnPrefix is set: use CDN URL
 * - If __LOCAL_STATIC_PREFIX__ is set (SSR without CDN): use that path
 * - Otherwise: use empty string (relative path)
 */
function getStaticBaseUrl(): string {
	if (globalThis.cdnPrefix) {
		return 'https://' + globalThis.cdnPrefix + '/js/dist';
	}
	// When SSR sets this, use the local path prefix to ensure proper routing through Nginx
	if (globalThis.__LOCAL_STATIC_PREFIX__) {
		return globalThis.__LOCAL_STATIC_PREFIX__;
	}
	return '';
}
...
export function lazyStylePropURL(name: string) {
	...
	return base ? `${base}/styleProperties/${name}.json` : `styleProperties/${name}.json`;
}

export function lazyCSSURL(name: string) {
	const base = getStaticBaseUrl();
	return base ? `${base}/css/${name}.css` : `css/${name}.css`;
}
