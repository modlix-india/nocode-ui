...
export function ScreenSizeSelector({ size, onChange }: SelectorProps) {
	const changeDeviceType = (event: any, deviceType: string) => {
		if (event.shiftKey) {
			if (!size.endsWith('_ONLY')) {
				onChange(deviceType);
			} else if (deviceType === 'WIDE_SCREEN') {
	...
	return (
		<div className="_propLabel _svgButtons" title="Screen Size">
			Screen:
				...
				>
					...
					>
						<g
							id="Group_59"
							data-name="Group 59"
							transform="translate(-1341.261 -237.23)"
						>
							<path
								id="Path_112"
								data-name="Path 112"
