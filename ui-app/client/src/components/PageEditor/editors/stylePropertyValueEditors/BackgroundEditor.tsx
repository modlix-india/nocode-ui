...
function BackgroundDetailedEditor({
	...
	const handleClipChange = (value: string) => {
		let clipValue = value;
		if (value === 'text') {
			clipValue = `${value}, color: transparent`;
		} else if (value === 'border-area') {
			clipValue = `${value}, border-color: transparent`;
		}
		onChangeCurry('Clip')(clipValue);
	};
	...
	return (
		<>
			...
			<div className="_combineEditors">
				...
				<div className="_simpleEditor">
					<IconsSimpleEditor
						...
						onChange={v => handleClipChange(v as string)}
						withBackground={true}
					/>
				</div>
			</div>
			...
			<div className="_combineEditors">
				<div className="_simpleEditor" style={{ width: '100%' }}>
					<ManyFunctionsEditor
...
function BackgroundStandardEditor({
	...
	const [imageType, setImageType] = useState<'URL' | 'Gradient'>('URL');
	const [showFileBrowser, setShowFileBrowser] = useState(false);
	const [backgroundImages, setBackgroundImages] = useState<
		Array<{ type: 'URL' | 'Gradient'; value: string }>
	>([]);
	...
	useEffect(() => {
		...
		const parsedImages = extractedImage.split(',').map((v: string) => {
			const trimmed = v.trim();
			if (trimmed.startsWith('url(')) {
				return { type: 'URL', value: trimmed.slice(4, -1) };
			} else {
				return { type: 'Gradient', value: trimmed };
			}
		});

		setBackgroundImages(
			parsedImages.length === 0 ? [{ type: 'URL', value: '' }] : parsedImages,
		);
	}, [selectedComponent, subComponentName, iterateProps, pseudoState, selectorPref]);
	...
	const handleImageTypeChange = (type: 'URL' | 'Gradient') => {
		setImageType(type);
		onChangeCurry('Image')('');
	};

	const handleImageChange = (value: string) => {
		if (imageType === 'URL' && !value.startsWith('url(')) {
			value = `url(${value})`;
		}
		onChangeCurry('Image')(value);
	};
	...
	const handleFileSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
		...
		if (file) {
			...
			reader.onload = e => {
				...
				handleImageChange(`url(${result})`);
			};
			reader.readAsDataURL(file);
		}
	};

	const getDefaultPosition = (x: string, y: string): [string, string] => {
		const xMap: Record<string, string> = { left: '0%', center: '50%', right: '100%' };
		const yMap: Record<string, string> = { top: '0%', center: '50%', bottom: '100%' };
		return [xMap[x as keyof typeof xMap] || x, yMap[y as keyof typeof yMap] || y];
	};

	const updateBackgroundImages = (newImages: Array<BackgroundImage>) => {
		const newValue = newImages
			.map(img => (img.type === 'URL' ? `url(${img.value})` : img.value))
			.join(', ');
		onChangeCurry('Image')(newValue);
				...
				{backgroundImages.map((img, index) => (
					<div key={`background_${index}`} className="_eachBackgroundEditor">
							...
							<IconsSimpleEditor
								...
								onChange={v => {
									if (v === 'Add' && canAddNewImage) {
										...
										newImages.splice(index, 1);
										newImages.splice(index - 1, 0, current);
										updateBackgroundImages(newImages);
									} else if (v === 'Move Down') {
								...
								options={[
									...
									{
										name: 'Move Down',
										description: 'Move this image layer down',
										width: '20',
										height: '14',
										icon: (
											<>
									...
									{
										name: 'Move Up',
										description: 'Move this image layer up',
										width: '20',
										height: '14',
										icon: (
											<>
						...
						<div className="_combineEditors">
							<div className="_simpleEditor">
								<div
									className="_imageInputContainer"
									style={{
										display: 'flex',
										alignItems: 'center',
										position: 'relative',
								...
								>
									{img.type === 'URL' && (
										<div
											className="_fileSelectorContainer"
											style={{
												position: 'absolute',
												left: '8px',
												top: '50%',
									...
									<input
										...
										style={{
											flex: 1,
											paddingLeft: img.type === 'URL' ? '30px' : '8px',
										}}
									/>
									<input
										type="file"
						...
						options={[
							{
								...
								icon: (
									<g transform="translate(9 9)">
										...
										/>
										<path
											d="M10.1477 12.7376C10.1774 12.7447 10.2082 12.7447 10.2379 12.7376C10.269 12.738 10.2998 12.7319 10.3284 12.7196C10.357 12.7074 10.3828 12.6893 10.4039 12.6665L11.035 12.0402C11.0456 12.0288 11.0585 12.0198 11.0728 12.0136C11.087 12"
											fill="#02B694"
											strokeOpacity={0}
										/>
										<path
				...
				<div className="_simpleEditor">
					<div className="_simpleEditorGroup">
						{[
							...
							<div key={rowIndex} className="_simpleEditor">
										...
										<>
											<button
												...
												onClick={() => {
													...
													const [defaultX, defaultY] = getDefaultPosition(
														x,
														y,
													);
