...
function BackgroundStandardEditor({
	...
	const updateSize = (newSize: string, newSizeX: string, newSizeY: string) => {
		...
		if (['cover', 'contain'].includes(newSize)) {
			onChangeCurry('Size')(newSize);
			onChangeCurry('SizeX')('');
			onChangeCurry('SizeY')('');
		} else if (newSize === 'auto') {
			onChangeCurry('Size')('auto auto');
			onChangeCurry('SizeX')(sizeX);
			onChangeCurry('SizeY')(sizeY);
		} else {
			let combinedSize = '';
			if (newSize === 'auto auto') {
				combinedSize = 'auto auto';
			} else if (newSizeX && newSizeY) {
				combinedSize = `${newSizeX} ${newSizeY}`;
			} else if (newSizeX) {
				combinedSize = `${newSizeX} auto`;
			} else if (newSizeY) {
				combinedSize = `auto ${newSizeY}`;
			} else {
				combinedSize = 'auto auto';
			}
