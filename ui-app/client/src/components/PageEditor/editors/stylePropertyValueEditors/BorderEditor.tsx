...
function BorderStandardEditor({
	...
	const onChangeBorderMode = (value: string | Array<string>) => {
		let newBorderMode: BorderMode[];
		if (Array.isArray(value)) {
			newBorderMode = value as BorderMode[];
		} else {
			newBorderMode = value.split(' ') as BorderMode[];
		}

		setBorderMode(newBorderMode);

		// Apply the changes based on the new border mode
		const newValues: { prop: string; value: string | ComponentProperty<string> }[] = [];

		if (newBorderMode.includes(BorderMode.ALL)) {
			// Apply the current values to all sides
			['Top', 'Right', 'Bottom', 'Left'].forEach(side => {
				['Width', 'Style', 'Color'].forEach(prop => {
					newValues.push({
						prop: `border${side}${prop}`,
						value:
							extractValue({
								subComponentName,
								prop: `border${prop}`,
			...
			// Apply the current values only to the selected sides
			newBorderMode.forEach(mode => {
				const side = mode[0] + mode.slice(1).toLowerCase();
				['Width', 'Style', 'Color'].forEach(prop => {
					newValues.push({
						prop: `border${side}${prop}`,
						value:
								...
								<>
									<g transform="translate(8 8)">
										<rect
											...
											strokeOpacity={0}
										/>
										<mask id="path-2-inside-1_3197_8784">
											<path
