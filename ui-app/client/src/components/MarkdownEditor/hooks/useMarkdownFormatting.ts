export function useMarkdownFormatting() {
    const formatText = (
        ...
        if (command.startsWith('style-')) {
            const styleValue = command.replace('style-', '');
            return {
                newText: `${beforeText}!!${selectedText}!!{style:"${styleValue}"}${afterText}`,
                newCursorPos: selection.end + 4 + styleValue.length
            };
        }
        ...
        const handleStyleFormat = (styleToAdd: string) => {
            ...
            let [property, value] = style.trim().split(':').map(s => s.trim());
            if (property && value) {
            ...
            newText = `${beforeText}!!${content}!!{style="${updatedStyles}"}${afterText}`;
            } else {
            newText = `${beforeText}!!${selectedText}!!{style="${styleToAdd}"}${afterText}`;
            }
