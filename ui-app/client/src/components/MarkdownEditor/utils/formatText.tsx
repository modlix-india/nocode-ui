export default function formatText(
	...
	const toggleFormat = (startMarker: string, endMarker: string = startMarker) => {
		const hasMarkers = selectedText.startsWith(startMarker) && selectedText.endsWith(endMarker);
		if (hasMarkers) {
			newText =
				beforeText + selectedText.slice(startMarker.length, -endMarker.length) + afterText;
