...
function ColorPickerComponent(props: ComponentProps) {
	...
	if (showColorPicker && !readOnly) {
		...
		if (buttonRef.current) {
			...
			if (rect.top + 400 > document.body.clientHeight)
				bodyPosition.bottom = document.body.clientHeight - rect.top;
		...
		const Dropdown: React.FC<DropdownProps> = ({
				...
				label = (
					<span className="_selectedOption">
						{Array.from(selection).map(
							s => options.find(e => e.name === s)?.displayName,
						)}
					</span>
				);
			} else {
			...
			const setCurrentOption = (num: number) => {
				...
				const options = ddBody.current.querySelectorAll(
					'._simpleColorPickerDropdownOption',
				);
			...
			if (open) {
				...
				body = (
					<div
						className="_simpleColorPickerDropdownBody"
						ref={ddBody}
	...
	return (
		<CommonInputText
			...
			customStyles={!designType?.startsWith('_box') ? undefined : colorStyle}
			cssPrefix={`comp compColorPicker ${readOnly ? '_disabled' : ''}`}
			noFloat={noFloat}
			label={!designType?.startsWith('_box') ? label : undefined}
			rightIcon={
			...
			onClickHandler={() => {
				if (designType?.startsWith('_box')) setShowColorPicker(true);
			}}
			onMouseEnter={() => {
			...
			focusHandler={() => {
				if (!readOnly && !designType?.startsWith('_box')) {
					setFocus(true);
