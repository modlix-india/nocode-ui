...
function applied(
	...
	userPicked: boolean,
	dec: ReturnType<typeof decideMode>,
) {
	return {
...
describe('partHasHtml / partHasDesign', () => {
	it('detects html and design for a part', () => {
		...
		expect(
			partHasDesign(emailTemplate({ design: { en: { body: [{ id: 'b' }] } } }), 'en', 'body'),
		).toBe(true);
		expect(partHasDesign(emailTemplate({ design: { en: { body: [] } } }), 'en', 'body')).toBe(
			false,
		);
	});
});
