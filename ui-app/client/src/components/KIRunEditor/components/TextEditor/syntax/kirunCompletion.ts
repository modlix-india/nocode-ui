...
// Module-level repository that can be updated
let currentFunctionRepository: Repository<Function> | undefined = undefined;

/**
 * Set the function repository for autocomplete
 * Call this when the repository changes (e.g., when switching contexts)
 */
export function setCompletionFunctionRepository(repo: Repository<Function> | undefined) {
	// Only update and clear cache if repository actually changed
	if (currentFunctionRepository !== repo) {
		currentFunctionRepository = repo;
		// Don't clear cache - the DSLFunctionProvider caches per-repository
		// so it will just use the cached results for this specific repo
	}
}

/**
 * Parse the function name from text before cursor when inside parentheses
 * Returns the full function name (e.g., "UIEngine.FetchData") or null if not in a function call
 */
function parseFunctionNameFromContext(textBeforeCursor: string): string | null {
	// Find the last opening parenthesis that doesn't have a matching close
	let parenDepth = 0;
	...
	// Extract the function name before the opening paren
	const textBeforeParen = textBeforeCursor.substring(0, lastOpenParenIndex);

	// Match function name pattern: Namespace.FunctionName or Namespace.SubNS.FunctionName
	const funcNameMatch = textBeforeParen.match(/([A-Za-z_][A-Za-z0-9_]*(?:\.[A-Za-z_][A-Za-z0-9_]*)+)\s*$/);
	if (funcNameMatch) {
...
/**
 * Get already used parameter names in the current function call
 */
function getUsedParameters(textBeforeCursor: string): Set<string> {
	...
	// Match parameter names (paramName = ...)
...
/**
 * Register autocomplete provider for KIRun DSL
 */
export function registerKIRunCompletionProvider() {
	monaco.languages.registerCompletionItemProvider('kirun-dsl', {
		...
		provideCompletionItems: async (model, position) => {
			...
			// Get all functions for autocomplete using the current repository
			...
			// Check if we're inside a function call (for parameter suggestions)
			const functionName = parseFunctionNameFromContext(textBeforeCursor);
			if (functionName) {
				// We're inside a function's parentheses - suggest parameters
			...
			if (textBeforeCursor.trim().endsWith('AS')) {
				// After AS keyword - suggest schema types
				suggestions.push(...getSchemaTypeSuggestions(range));
			} else if (textBeforeCursor.includes(':') && !textBeforeCursor.trim().startsWith('//')) {
				// After colon - suggest function names
				suggestions.push(...getFunctionSuggestions(functions, range));
			} else if (
				textBeforeCursor.trim() === '' ||
				textBeforeCursor.trim().endsWith('\n')
			) {
				// Start of line - suggest keywords and block names
...
/**
 * Get schema type suggestions
 */
function getSchemaTypeSuggestions(
	range: monaco.IRange,
): monaco.languages.CompletionItem[] {
	const types = [
...
function getParameterSuggestions(
	...
	return func.parameters
		...
		.map((param, index) => ({
			...
			range,
			sortText: String(index).padStart(3, '0'), // Keep original parameter order
		}));
}

/**
 * Get block name suggestions
 */
function getBlockNameSuggestions(
	range: monaco.IRange,
): monaco.languages.CompletionItem[] {
	const blocks = [
