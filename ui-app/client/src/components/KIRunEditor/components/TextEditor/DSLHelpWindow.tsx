...
/**
 * DSL Help Window Component
 * Provides comprehensive documentation for the KIRun DSL
 */
		...
		overview: {
			...
			content: (
				<div className="_help-content">
					...
					<p>
						KIRun Language is a text-based language for defining functions in the KIRun runtime.
						It provides a more readable and maintainable way to write function logic compared to visual editors.
					</p>
					...
					<ul>
						...
						<li>Nested blocks for control flow (true, false, iteration, output, error)</li>
				...
				<div className="_help-content">
					...
					<table className="_keyword-table">
						...
						<tbody>
							<tr>
								...
								<td><code>FUNCTION myFunction</code></td>
							</tr>
							<tr>
								<td><code>NAMESPACE</code></td>
								<td>Optional namespace for the function</td>
								<td><code>NAMESPACE MyApp.Utils</code></td>
							</tr>
							<tr>
					...
					<table className="_keyword-table">
						...
						<tbody>
							...
							<tr>
								<td><code>WITH DEFAULT VALUE</code></td>
					...
					<table className="_keyword-table">
						...
						<tbody>
							<tr>
								...
								<td><code>AFTER Steps.step1.output</code></td>
							</tr>
							<tr>
								<td><code>IF</code></td>
								<td>Conditional execution</td>
								<td><code>IF Steps.check.true</code></td>
							</tr>
						</tbody>
					</table>
				</div>
					...
					<table className="_keyword-table">
						...
						<tbody>
							<tr>
								...
								<td><code>ARRAY OF TYPE</code></td>
								<td><code>items AS ARRAY OF INTEGER</code></td>
							</tr>
							<tr>
								<td><code>OBJECT</code></td>
								<td><code>OBJECT</code> or JSON schema</td>
								<td><code>data AS OBJECT</code></td>
							</tr>
							<tr>
								<td>JSON Schema</td>
								<td>Inline JSON object</td>
								<td><code>config AS {`{type: "STRING"}`}</code></td>
							</tr>
						</tbody>
					</table>
...
empty: SetValue(value = null)`}</pre>
					...
					<p>Use backticks or single quotes for expressions that will be evaluated:</p>
...
data: SetData(value = {
    ...
    tags: ["admin", "user"]
					...
					<table className="_keyword-table">
						...
						<tbody>
							<tr>
								<td><code>Arguments.name</code></td>
								<td>Function input parameters</td>
								<td><code>Arguments.userId</code></td>
							</tr>
							<tr>
								<td><code>Steps.name.output</code></td>
								<td>Statement output</td>
								<td><code>Steps.calc.output.result</code></td>
							</tr>
							<tr>
								<td><code>Steps.name.eventName</code></td>
								<td>Statement event output</td>
								<td><code>Steps.fetch.success.data</code></td>
							</tr>
							<tr>
								<td><code>Steps.name.iteration</code></td>
								<td>Loop iteration data</td>
								<td><code>Steps.loop.iteration.index</code></td>
							</tr>
							<tr>
								<td><code>Context.name</code></td>
								<td>Context variable</td>
								<td><code>Context.tempData</code></td>
							</tr>
						</tbody>
					</table>
...
finalize: Complete() IF Steps.check1.true, Steps.check2.true /* Multiple conditions */`}</pre>
					...
					<ul>
						<li><strong>Parallel execution:</strong> Steps without dependencies run in parallel</li>
						<li><strong>Sequential execution:</strong> Use AFTER to ensure order</li>
						<li><strong>Conditional execution:</strong> Use IF to skip steps based on conditions</li>
						<li><strong>Event-based flow:</strong> Branch logic based on event outputs (true/false/error)</li>
					</ul>
				</div>
			),
		},
		...
		blocks: {
			...
			content: (
				<div className="_help-content">
					<h3>Block Types</h3>
					<p>Statements can contain nested blocks for different execution contexts:</p>
    ...
    output
        transform: TransformResult(data = Steps.main.output.result)`}</pre>
					...
					<p>Blocks must be indented relative to their parent statement. Use consistent indentation (spaces or tabs).</p>
	...
	return (
		<div className={`_dsl-help-container _theme-${theme}`} data-theme={theme}>
			...
			<div className="_help-footer">
				<p>
					<strong>Quick Tips:</strong> Use backticks ` for expressions, double quotes " for literal strings.
					Comments go after the statement with /* */. Press <kbd>Ctrl+Space</kbd> for autocomplete.
				</p>
			</div>
		</div>
	);
